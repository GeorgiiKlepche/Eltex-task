...
static void handle_msg(union sigval sv) {
  ...
  while (ret = mq_receive(q_handler->rcv_q, q_handler->buf, USER_SIZE, &prio) >
               0) {
...
int client() {
  ...
  q_handler.main_wnd =
      derwin(q_handler.wnd, (size.ws_row - 7), (size.ws_col / 4) * 3 - 1, 1, 1);
  q_handler.users_wnd =
      derwin(q_handler.wnd, size.ws_row - 2, (size.ws_col / 4) - 2, 1,
             (size.ws_col / 4) * 3 + 1);
  q_handler.msg_wnd =
      derwin(q_handler.wnd, 4, (size.ws_col / 4) * 3 - 1, (size.ws_row - 5), 1);
  ...
  while (1) {
    if (mq_receive(login_q, tmp_buf, LOGIN_SIZE, &prio) == -1) {
      ...
      if (USER_PRIORITY == prio) {
        ...
        ret = mq_send(user.snd_q, user.login, LOGIN_SIZE, USRNAME_PRIORITY);
        if (ret == -1) {
          if (errno != EAGAIN) {
        ...
        while (ret =
                   mq_receive(user.rcv_q, user.rcv_buf, USER_SIZE, &prio) > 0) {
          if (MSG_PRIORYTY == prio) {
            wprintw(q_handler.main_wnd, "%s\n", q_handler.buf);
            wrefresh(q_handler.main_wnd);
          }
