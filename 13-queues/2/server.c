...
int server() {
  ...
  login_q = mq_open(LOGIN_NAME, O_CREAT | O_RDWR | O_NONBLOCK, PERM, &login_attr);
  if (login_q == -1) {
  ...
  while (1) {
    if ((ret = mq_receive(login_q, tmp_buf, LOGIN_SIZE, &prio)) == -1) {
      ...
      if (SERVER_PRIORYTY == prio) {
        ...
        snprintf(users[users_count - 1].login, LOGIN_SIZE, "Пользователь%d", users[users_count - 1].id);
        snprintf(users[users_count - 1].stc_ch, LOGIN_SIZE, "/client%d_stoc", users[users_count - 1].id);
        snprintf(users[users_count - 1].cts_ch, LOGIN_SIZE, "/client%d_ctos", users[users_count - 1].id);

        printf("Зарегистрирован \"%s\" как пользователь #%d с %s и %s каналами\n",
               users[users_count - 1].login, users[users_count - 1].id,
               users[users_count - 1].stc_ch, users[users_count - 1].cts_ch);

        users[users_count - 1].rcv_q = mq_open(users[users_count - 1].cts_ch, O_CREAT | O_RDONLY | O_NONBLOCK, PERM, &user_attr);
        if (users[users_count - 1].rcv_q == -1) {
          perror("rcv_q mq_open");
          exit(1);
        }
        users[users_count - 1].snd_q = mq_open(users[users_count - 1].stc_ch, O_CREAT | O_WRONLY | O_NONBLOCK, PERM, &user_attr);
        ...
        strncat(users_list, tmp_buf, USER_SIZE - strlen(users_list) - 1);
        for (index = 0; index < users_count; ++index) {
          if (users[index].id == -1) continue;
          mq_send(users[index].snd_q, users_list, USER_SIZE, UPDUSER_PRIORITY);
        }
      }
    }
    ...
    for (index = 0; index < users_count; ++index) {
      if (users[index].id == -1) continue;
      if ((ret = mq_receive(users[index].rcv_q, users[index].rcv_buf, USER_SIZE, &prio)) == -1) {
        if (errno != EAGAIN) {
        ...
        if (USER_PRIORITY == prio) {
          ...
          snprintf(tmp_buf, USER_SIZE, "%s: %s", users[index].login, users[index].rcv_buf);
          for (int sec_index = 0; sec_index < users_count; ++sec_index) {
            if (users[sec_index].id != -1) {
              mq_send(users[sec_index].snd_q, tmp_buf, USER_SIZE, MSG_PRIORYTY);
            }
          }
        } else if (USRNAME_PRIORITY == prio) {
          strncpy(users[index].login, users[index].rcv_buf, LOGIN_SIZE - 1);
          ...
          for (int sec_index = 0; sec_index < users_count; ++sec_index) {
            if (users[sec_index].id == -1) continue;
            mq_send(users[sec_index].snd_q, users_list, USER_SIZE, UPDUSER_PRIORITY);
          }
