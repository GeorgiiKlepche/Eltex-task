...
int main() {
  ...
  if (bind_status == -1) {
    handle_error("ERR bind", passive_sock, -1);
  }

  if (listen(passive_sock, 2) == -1) {
    handle_error("ERR listen", passive_sock, -1);
  }

  active_sock =
      accept(passive_sock, (struct sockaddr *)&client_addr, &addr_size);
  if (active_sock == -1) {
    handle_error("ERR accept", passive_sock, -1);
  }

  int recv_status = recv(active_sock, buffer, 10, 0);
  if (recv_status == -1) {
    handle_error("ERR recv", passive_sock, active_sock);
  }
  ...
  if (send_status == -1) {
    handle_error("ERR send", passive_sock, active_sock);
  }
